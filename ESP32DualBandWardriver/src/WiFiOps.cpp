...
// First 14 entries (idx 0..13) are 2.4 GHz, the remaining 26 are 5 GHz.
// scan_channels is not declared constexpr so a static_assert would not
// compile — invariant is enforced by code review of this and other touches
// to the array.
#define WARDRIVE_2_4_BAND_END_IDX 13

uint8_t assigned_start_idx = 0;
#ifdef WARDRIVE_2_4_ONLY
// On 2.4-only hardware, do not advertise the 5-GHz slice before the CORE's
// first MSG_ADMIN arrives — otherwise the boot-time scan attempts spam
// `wifi:unsupported channel` errors and stall ESP-NOW heartbeat sends.
uint8_t assigned_end_idx = WARDRIVE_2_4_BAND_END_IDX;
#else
...
void WiFiOps::startESPNow() {
  ...
  // Broadcast-Peer (FF:FF:...) EINMALIG anlegen. Sonst liefert esp_now_send an
  // die Broadcast-Adresse ESP_ERR_ESPNOW_NOT_FOUND — bisher wurde der Peer nur
  // lazy im Scan-Pfad (sendBroadcastStringPlain) angelegt, was in der Lobby
  // weggegatet ist => alle Lobby-Heartbeats floppen und die Node registriert
  // sich nie beim CORE. Hier angelegt gilt er fuer Heartbeat UND Records.
  {
    ...
    if (!esp_now_is_peer_exist(bcast_mac)) {
      ...
      p.encrypt = false;   // plaintext broadcast
      esp_now_add_peer(&p);
    }
  }
...
int WiFiOps::runWardrive(uint32_t currentTime) {
  ...
  // Barrier: a NODE stays in the Lobby until the CORE issues SESSION_CMD_START.
  ...
  if ((this->run_mode == SOLO_MODE) || (this->run_mode == NODE_MODE)) {
    ...
    if (((gps.getGpsModuleStatus()) && (gps.getFixStatus()) && (sd_obj.supported)) || 
      ...
      if (scan_status == WIFI_SCAN_RUNNING) // Scan is still running
        ...
        // Scan BLE here — nur EINE Node scannt BLE, sonst ueberlappen sich die
        // Nodes und der CORE bekaeme dasselbe Geraet mehrfach (er dedupliziert
...
#ifdef C5_ZERO_NODE
  // WS2812 status LED on GPIO27: toggle dim green every scan cycle so the
  // headless Zero node visibly shows it is alive and sweeping channels.
  // (The existing LED_PIN logic is SOLO_MODE-only, so a NODE stays dark.)
  static bool s_zero_led = false;
  s_zero_led = !s_zero_led;
  rgbLedWrite(27, 0, s_zero_led ? 24 : 0, 0);
#endif
  ...
  if (networks > 0) {
    for (int i = 0; i < networks; i++) {
      ...
      if (this->run_mode == SOLO_MODE) {
        ...
        // back-to-back broadcast burst (each send here re-runs setFixedChannel
        // + esp_now_send with no wait), so without this gap the whole scan
        // batch is dropped and — being deduped — never resent, leaving the CORE
        // with ~1 WiFi while sparse, repeated BLE gets through. Let each packet
        // drain before the next channel toggle clobbers it.
        delay(20);
      }
    }
  }
...
bool WiFiOps::begin(bool skip_admin) {
  ...
  // Compile-time role override. The wardriver upstream lets run_mode be set
  // at runtime via SPIFFS (default 1 = SOLO_MODE), but for the headless
  // NodeMCU-32 port we want NODE_MODE always — no web-admin available to
  // flip the mode, and a fresh SPIFFS would boot SOLO and never speak
  // ESP-NOW to the HUB. The same #defines that gate the pin map decide here.
#ifdef NODE
  this->run_mode = NODE_MODE;
#elif defined(CORE)
  this->run_mode = CORE_MODE;
#endif
