...
void BatteryInterface::RunSetup() {
  ...
  // C5_ZERO_NODE: bare Waveshare Zero has no battery monitor and no I2C
  // pull-ups on 4/5. Probing the floating bus CPU_LOCKUPs the C5. Skip it —
  // getBatteryLevel() stays safe (has_* flags remain false → returns 0).
  #if defined(HAS_BATTERY) && !defined(C5_ZERO_NODE)
