...
void WiFiScan::StartScan(uint8_t scan_mode, uint16_t color) {  
  ...
  #ifdef MARAUDER_CORE_MODE
  else if (scan_mode == WIFI_SCAN_WAR_DRIVE_CORE) {
    RunWardriveCore(color);
  }
  #endif
  #ifdef MARAUDER_WDGWARS_UPLOAD
  else if (scan_mode == WIFI_SCAN_WDGWARS_UPLOAD) {
    RunWdgwarsUpload(color);
  }
  #endif
  #ifdef MARAUDER_FILE_SERVER_AP
  else if (scan_mode == WIFI_SCAN_FILE_SERVER_AP) {
    RunFileServerAP(color);
  }
  #endif
...
bool WiFiScan::shutdownWiFi() {
  // CORE-Mode-Cleanup-Hook: wenn shutdownWiFi() von einem anderen Mode-Switch
  // aus gerufen wird (z.B. OTA-Pfad), MUESSEN wir esp_now_deinit() rufen,
  // sonst hinterbleibt PMK/Peer-State im IDF. Top-3-Risiko 2+3 in
  // integration_design.md Anhang B.
  #ifdef MARAUDER_CORE_MODE
    if (this->currentScanMode == WIFI_SCAN_WAR_DRIVE_CORE && wardrive_core_obj.isRunning()) {
  ...
  #endif
  #ifdef MARAUDER_WDGWARS_UPLOAD
    if (this->currentScanMode == WIFI_SCAN_WDGWARS_UPLOAD && wdgwars_upload_obj.isActive()) {
      wdgwars_upload_obj.deinit();
    }
  #endif
  #ifdef MARAUDER_FILE_SERVER_AP
    if (this->currentScanMode == WIFI_SCAN_FILE_SERVER_AP && file_server_ap_obj.isActive()) {
      file_server_ap_obj.deinit();
    }
  #endif
...
void WiFiScan::StopScan(uint8_t scan_mode) {
  ...
  if (currentScanMode == WIFI_SCAN_WAR_DRIVE
      #ifdef MARAUDER_CORE_MODE
      || currentScanMode == WIFI_SCAN_WAR_DRIVE_CORE
      #endif
     )
    this->closePoiFile();
...
void WiFiScan::RunEapolScan(uint8_t scan_mode, uint16_t color) {
  ...
  // Active/Targeted EAPOL-Modi implizieren Deauth — sonst sind sie funktional
  // identisch zum passiven Scan. Der ForcePMKID-Setting-Toggle steuert nur,
  // ob der passive Mode aktiv wird.
  if (scan_mode == WIFI_SCAN_ACTIVE_EAPOL ||
      scan_mode == WIFI_SCAN_ACTIVE_LIST_EAPOL) {
    this->send_deauth = true;
  }
  ...
  /*#ifdef HAS_ILI9341
    ...
    startPcap("eapol");
      ...
      this->setupScanDisplayArea(TFT_WHITE, color);
      #ifdef HAS_FULL_SCREEN
        const char* eapol_title;
        if (scan_mode == WIFI_SCAN_ACTIVE_EAPOL)        eapol_title = "Active PMKID";
        else if (scan_mode == WIFI_SCAN_ACTIVE_LIST_EAPOL) eapol_title = "Targeted PMKID";
        else                                              eapol_title = "EAPOL Sniff";
        display_obj.tft.fillRect(0,16,TFT_WIDTH,16, color);
        display_obj.tft.drawCentreString(eapol_title,TFT_WIDTH / 2,16,2);
      #endif
      display_obj.tft.setTextColor(TFT_GREEN, TFT_BLACK);
      display_obj.tftDrawChannelScaleButtons(set_channel, false);
      display_obj.tftDrawExitScaleButtons(false);
...
#ifdef MARAUDER_CORE_MODE
// RunWardriveCore: Trigger-Funktion fuer Core-Mode. Gerufen aus StartScan().
// Adapted from RunBeaconScan-WAR_DRIVE-Pfad oben + WiFiOps::startESPNow.
void WiFiScan::RunWardriveCore(uint16_t color) {
  // GPS-Modul Pflicht (no-fix ist OK, no-module nicht).
  #ifdef HAS_GPS
    if (!gps_obj.getGpsModuleStatus()) {
      Serial.println("CORE: GPS module not detected, abort");
      return;
    }
  #else
    Serial.println("CORE: HAS_GPS not defined, abort");
    return;
  #endif

  // Heap-Reserve.
  if (ESP.getFreeHeap() < WARDRIVE_CORE_HEAP_MIN_INIT) {
    Serial.printf("CORE: Low memory (%u < %u), abort\n",
                  ESP.getFreeHeap(), (unsigned)WARDRIVE_CORE_HEAP_MIN_INIT);
    return;
  }
...
#ifdef MARAUDER_WDGWARS_UPLOAD
// RunWdgwarsUpload: Trigger-Funktion fuer den HTTPS-Uploader.
// Gerufen aus StartScan(). Eintritts-Voraussetzung: kein anderer Mode aktiv.
void WiFiScan::RunWdgwarsUpload(uint16_t color) {
  this->setLEDMode(MODE_SNIFF);
    ...
    #ifdef HAS_FULL_SCREEN
      display_obj.tft.fillRect(0, 16, TFT_WIDTH, 16, color);
      display_obj.tft.drawCentreString("WDGWars Upload", TFT_WIDTH / 2, 16, 2);
    #endif
  #endif

  this->currentScanMode = WIFI_SCAN_WDGWARS_UPLOAD;
  wdgwars_upload_obj.init();
  this->wifi_initialized = true;
  initTime = millis();
}

void WiFiScan::shutdownWdgwarsUpload() {
  if (wdgwars_upload_obj.isActive()) {
    wdgwars_upload_obj.deinit();
  }
}
...
#ifdef MARAUDER_FILE_SERVER_AP
// RunFileServerAP: Trigger-Funktion fuer den SD-File-Server.
// Gerufen aus StartScan(). Eintritts-Voraussetzung: kein anderer Mode aktiv.
void WiFiScan::RunFileServerAP(uint16_t color) {
  this->setLEDMode(MODE_SNIFF);
    ...
    #ifdef HAS_FULL_SCREEN
      display_obj.tft.fillRect(0, 16, TFT_WIDTH, 16, color);
      display_obj.tft.drawCentreString("File Server", TFT_WIDTH / 2, 16, 2);
    #endif
  #endif

  this->currentScanMode = WIFI_SCAN_FILE_SERVER_AP;
  file_server_ap_obj.init();
  this->wifi_initialized = true;
  initTime = millis();
}

void WiFiScan::shutdownFileServerAP() {
  if (file_server_ap_obj.isActive()) {
    file_server_ap_obj.deinit();
  }
}
...
void WiFiScan::renderRawStats() {
    ...
    if ((this->currentScanMode == WIFI_SCAN_EAPOL) ||
        ...
        (this->currentScanMode == WIFI_SCAN_ACTIVE_LIST_EAPOL)) {
      display_obj.tft.setTextColor(TFT_MAGENTA, TFT_BLACK);
...
{
  // Top-Branch fuer MARAUDER_CORE_MODE — MUSS am Anfang stehen, mit `return`
  // direkt nach runTick(), damit die anderen Mode-Branches NICHT parallel
  // ausgefuehrt werden. Top-3-Risiko 1 in integration_design.md Anhang B.
  // TODO: verify branch position survives Marauder-upstream-rebases.
  #ifdef MARAUDER_CORE_MODE
    if (currentScanMode == WIFI_SCAN_WAR_DRIVE_CORE) {
      wardrive_core_obj.runTick(currentTime);
      return;
    }
  #endif

  // WDGWars-Upload-Branch — eigener State-Machine-Pfad, blocking-IO erlaubt
  // weil keine zeitkritische RX-Pipeline. Unused-currentTime-Param ist okay.
  #ifdef MARAUDER_WDGWARS_UPLOAD
    if (currentScanMode == WIFI_SCAN_WDGWARS_UPLOAD) {
      wdgwars_upload_obj.runTick();
      return;
    }
  #endif

  // File-Server-AP branch — same shape as WDGWARS: own state machine, no
  // RX hot-path. Hand the loop to the AP module until Center-long-press
  // exits.
  #ifdef MARAUDER_FILE_SERVER_AP
    if (currentScanMode == WIFI_SCAN_FILE_SERVER_AP) {
      file_server_ap_obj.runTick();
      return;
    }
  #endif
  ...
  (currentScanMode == WIFI_SCAN_ALL))
  {
    ...
    if (currentTime - initTime >= this->channel_hop_delay * HOP_DELAY) {
      ...
      if (currentScanMode == BT_SCAN_FLOCK) {
        this->channelHop();
        // warroom-rig: passive rig -- Flock-beacon transmit removed
      }
    }
