...
// 90-92 for the Marauder Core Mode fork. Moved down from 84-86 after upstream
// v1.13.0 claimed 84 for BT_SCAN_FOX_HUNT. These are internal scan-mode enum
// values only (never on any wire), so the exact numbers are free — they just
// need to stay unique and clear of upstream's range (85-98 is the free gap).
#define WIFI_SCAN_WAR_DRIVE_CORE 90
#define WIFI_SCAN_WDGWARS_UPLOAD 91
#define WIFI_SCAN_FILE_SERVER_AP 92
...
class WiFiScan
{
  ...
  public:
    ...
    void StopScan(uint8_t scan_mode);
      ...
      void shutdownWdgwarsUpload();
    #endif
    #ifdef MARAUDER_FILE_SERVER_AP
      void RunFileServerAP(uint16_t color);
      void shutdownFileServerAP();
    #endif
